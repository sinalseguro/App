...
- `npm run test:panic-trigger`, `npm run test:remote-sync-status`, `npm run test:owner-auto-call`, `npm run test:media-handoff`, `npm run test:media-processing-status` e `npm run test:finish-outcome` cobrem essas decisoes sem abrir camera, WebRTC real, API, UI ou backend.
