...
- Validacao Android fisica da Frente 1.3 executada no modelo `23129RA5FL` com identificador do aparelho redigido: APK SHA-256 `19ad59c4b9c4c47c8316f3a24d354626ee11a3442be910841fcd1e73283cd08b` instalado com sucesso, telas `Perfis`, `Anjos` e `Convite recebido` capturadas por ADB, sem crash fatal no recorte de log saneado.
- Release privado Android publicado no portal publico: `https://www.sinalseguro.com.br/downloads/private/android/sinalseguro_android.apk`, QR estavel `https://www.sinalseguro.com.br/assets/app/sinalseguro-android-qr.svg`, release EC2 `/var/www/sinalseguro/releases/20260513T215810Z`, versao `0.1.0` atualizada em `13/05/2026`.
...
- Correcao de politica: APK/AAB/IPA privados nao devem ficar rastreados no Git. O APK Android fica como artefato local de deploy e publicacao EC2/portal; Git guarda QR, manifesto, checksums, scripts, README e docs.
- UX publica de download: nao usar termos internos como frentes, release, EC2, manifesto ou checksum nas telas publicas; manter fluxo em ate tres interacoes e preservar QR Android/nome `sinalseguro_android.apk` estaveis.
- Validacao visual complementar Tarcila/Lina/Eliane em `docs/evidencias/android/2026-05-13-frente-1-3-visual-tarcila/`: fonte normal e fonte 1.3 para `Perfis`, `Anjos` e `Convite recebido`, sem padroes fatais no crash scan e sem evidencias sensiveis preservadas. A fonte 1.3 ficou documentada com ressalva de UX por cortes/overflow em textos longos.
