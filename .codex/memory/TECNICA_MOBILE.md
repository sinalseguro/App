...
# Atualizacao - 2026-05-21 - Etapas 1.119 e 1.120 Configuracoes

- `settingsPresentationPolicy` agora centraliza tambem o modelo dos cards principais da tela `Configuracoes`.
- Etapa 1.119: `buildSettingsDashboardTileRows()` define as linhas dos 8 cards com labels, descricoes, icones simbolicos e acoes.
- Etapa 1.120: `buildSettingsDashboardTileAction()` centraliza o alvo puro de painel dos cards.
...
# Atualizacao - 2026-05-22 - Etapas 1.127 e 1.128 Configuracoes

- `settingsPresentationPolicy` agora centraliza tambem acoes tipadas dos paineis de atualizacao e login.
- Etapa 1.127: `buildSettingsUpdatePanelState()` retorna as acoes `verify-update` e `download-update`.
- Etapa 1.128: `buildSettingsLoginPanelState()` retorna acoes de validar sessao, sair, entrar por e-mail, testar API, Google e Apple.
- `app/configuracoes.tsx` segue responsavel por executar os efeitos reais; `handleUpdatePanelAction()` e `handleLoginPanelAction()` apenas roteiam intencoes tipadas para handlers existentes.
...
# Atualizacao - 2026-05-22 - Etapas 1.137 e 1.138 Feedback de Alertas recebidos

- `receivedAlertPresentationPolicy` agora centraliza status, labels e dialogs puros de feedback da tela `Alertas recebidos`.
- Etapa 1.137: extraidos status de refresh, labels de resposta ao pedido e mensagens de estado da chamada recebida.
- Etapa 1.138: extraidos dialogs de falha e fallback de erro para pedido, arquivo local e tempo real.
- `app/alerta.tsx` segue responsavel por efeitos reais: API, autoaceite, notificacao, WebRTC, storage seguro, refs mutaveis, reset de chamada, Share nativo e estado React.
- Gate focado: `npm run test:received-alert-presentation`.
...
# Atualizacao - 2026-05-22 - Etapas 1.139 e 1.140 Acoes e historico de Alertas recebidos

- `receivedAlertPresentationPolicy` agora centraliza estado puro de acoes por alerta e apresentacao pura dos cards de historico local.
- Etapa 1.139: `buildReceivedAlertActionState()` calcula aceite remoto/local, sessao do painel, outra chamada ativa e bloqueios visuais sem executar efeitos.
- Etapa 1.140: `buildReceivedCallArchiveCardPresentation()` monta labels de snapshot, pessoa protegida, data, duracao, status e restricao legal usando os formatadores existentes.
- `app/alerta.tsx` segue responsavel por efeitos reais: API, autoaceite, notificacao, WebRTC, storage seguro, refs mutaveis, reset de chamada, Share nativo, selecao de registro e estado React.
- Gate focado: `npm run test:received-alert-presentation`.
- Validacoes aprovadas: teste focado, `test:received-alert-runtime`, `smoke-test`, `lint`, `private:android:readiness`, `npm test` e `git diff --check`.
- `typecheck` nao emitiu erro, mas ficou sem saida e ocioso por mais de 1 minuto; foi encerrado para nao deixar processo pendurado.
- Sem build/instalacao Android porque a fatia e pura e nao altera runtime fisico.
...
# Atualizacao - 2026-05-22 - Etapas 1.143 e 1.144 Status e secao de historico de Alertas recebidos

- `app/alerta.tsx` agora possui `ReceivedAlertsStatusBar` e `ReceivedCallArchiveSection` como componentes locais de apresentacao.
- Etapa 1.143: a barra de status/refresh recebe `status`, `refreshing` e `onRefresh` do `AlertScreen`.
- Etapa 1.144: a secao de historico recebe `records`, `onOpenRecord` e `onShareRecord` do `AlertScreen`.
- `AlertScreen` segue responsavel por API, autoaceite, notificacao, WebRTC, storage seguro, refs mutaveis, reset de chamada, Share nativo, selecao de registro, refresh e estado React.
- Gate focado: `npm run test:received-alert-presentation`.
- Validacoes aprovadas: teste focado, `test:received-alert-runtime`, `smoke-test`, `lint`, `private:android:readiness`, `npm test` e `git diff --check`.
- `typecheck` nao emitiu erro, mas ficou sem saida e ocioso por mais de 1 minuto; foi encerrado para nao deixar processo pendurado.
- Sem build/instalacao Android porque a fatia e presentational e nao altera runtime fisico.
...
# Atualizacao - 2026-05-22 - Etapas 1.151 e 1.152 Dialogs visuais de Anjos

- `app/contatos.tsx` agora possui dialogs locais de apresentacao: `TrustedAngelsInviteDialog`, `TrustedAngelsProfileBlockDialog`, `TrustedAngelsRevokeInvitationDialog` e `TrustedAngelsRevokeContactDialog`.
- Etapa 1.151: os dialogs de convite e bloqueio de perfil recebem labels, mensagem, valor do campo e callbacks ja preparados pelo `ContactsScreen`.
- Etapa 1.152: os dialogs de revogacao recebem label e callback de confirmacao ja definidos pela tela.
- `ContactsScreen` segue responsavel por gate de perfil, API, cache local, device binding, AppState, Share, criacao de convite, revogacoes reais, `setDialog`, navegacao e estado React.
- Gate focado: `npm run test:trusted-angels-dialog`, `test:trusted-angels-action` e `test:trusted-angels-panel`.
- Validacoes aprovadas: testes focados, `smoke-test`, `lint`, `typecheck`, `private:android:readiness`, `npm test` e `git diff --check`.
...
# Atualizacao - 2026-05-22 - Etapas 1.153 e 1.154 Dialogs de Estado e Prontidao de Anjos

- `app/contatos.tsx` agora possui `TrustedAngelsStateDialog` e `TrustedAngelsReadinessDialog` como wrappers locais de apresentacao.
- Etapa 1.153: o dialog de estado/resumo recebe `notice`, `status`, `visible` e callback de fechamento ja preparados pelo `ContactsScreen`.
- Etapa 1.154: o dialog de prontidao recebe `readinessState`, `visible` e callback de fechamento ja preparados pela tela.
- `ContactsScreen` segue responsavel por gate de perfil, API, cache local, device binding, AppState, Share, criacao de convite, revogacoes reais, `setDialog`, `setPanel`, navegacao e estado React.
- Gate focado: `npm run test:trusted-angels-dialog`, `test:trusted-angels-panel` e `test:trusted-angels-dashboard`.
- Validacoes aprovadas: testes focados, `smoke-test`, `lint`, `typecheck`, `private:android:readiness`, `npm test` e `git diff --check`.
- `private:android:readiness` manteve a pendencia local conhecida de Node 20.16.0 para release publico, aceitavel para build privado debug.
- Sem build/instalacao Android porque a fatia e presentational e nao altera runtime fisico.
